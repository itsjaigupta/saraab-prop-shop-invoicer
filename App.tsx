...
import { Printer, Copy, Check, Save, FolderOpen, Plus, Receipt, BarChart2, Download, Upload, FileText, TrendingUp } from 'lucide-react';
...
const getEmptyInvoice = (id = '001'): Invoice => {
  let logo = null;
  ...
  if (typeof window !== 'undefined') {
    try {
      const savedDefaults = localStorage.getItem('saraab_branding_defaults');
      if (savedDefaults) {
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('saraab_invoices');
    if (stored) {
        try {
    ...
    const storedExp = localStorage.getItem('saraab_expenses');
    if (storedExp) {
  ...
  const handleSaveInvoice = () => {
    ...
    localStorage.setItem('saraab_invoices', JSON.stringify(updatedList));
  ...
  const handleSaveDefaults = () => {
    ...
    localStorage.setItem('saraab_branding_defaults', JSON.stringify(defaults));
  ...
  const handleNewInvoice = () => {
    ...
    if (window.confirm("Start a new invoice? Any unsaved changes to the current invoice will be lost.")) {
        setInvoice(getEmptyInvoice(nextIdStr));
        setView('edit');
    }
  };
  ...
  const handleDeleteInvoice = (id: string, e: React.MouseEvent) => {
      ...
      if (window.confirm(`Are you sure you want to delete Invoice #${id}?`)) {
          ...
          localStorage.setItem('saraab_invoices', JSON.stringify(updated));
      }
  };
  ...
  const handleAddExpense = (exp: Expense) => {
    ...
    setExpenses(updated);
    localStorage.setItem('saraab_expenses', JSON.stringify(updated));
  };
      ...
      <nav className="bg-black text-white shadow-xl sticky top-0 z-50 no-print">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar max-w-full pb-1 sm:pb-0 pl-4 mask-fade-left">
